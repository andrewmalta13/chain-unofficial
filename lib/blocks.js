function Blocks(opts){
  ...
  function Propogate(blockHex, callback){
    callback({"err": "chain does not provide support for propogating blocks"}, null);
  }
    ...
    ids.forEach(function (blockId){
      opts.chain.getBlock(blockId, function (err, resp){
        if (err){
          ...
          var response = {blockId: resp.hash, result: []};

          resp.transaction_hashes.forEach(function (txid){
            var txPair = {txid: txid, txId: txid};
            response.result.push(txPair);
          });
