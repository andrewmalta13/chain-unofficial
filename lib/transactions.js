...
function standardizeTransaction(resp){
  ...
  resp.inputs.forEach(function (input){
    response.vin.push({txid: input.transaction_hash, txId: input.transaction_has, vout: input.output_index, 
                        scriptSig: {asm: null, hex: input.script_signature}, sequence: null});
  });
  resp.outputs.forEach(function (output){
    response.vout.push({value: output.value, index: output.output_index, spentTxid: output.spending_transaction || null,
                       scriptPubKey : {asm: null, hex: output.script_hex, reqSigs: output.required_signatures,
...
var Transactions = function(opts){
  ...
  function Get(transactions, callback){
    ...
    if (transactions.txids) txs = transactions.txids;
    else if (transactions.txIds) txs = transactions.txIds;
  ...
  //returns the latest unconfirmed transaction propogated to chain as a json object. 
  ...
  function Propogate(transactionHex, callback){
    ...
    chain.sendTransaction(hex, function (err, resp) {
      if (err){
        ...
        callback(false, resp.transaction_hash);
      }
    });
  }
  ...
  function Status(txids, callback){
    var ids;
    var response = [];
    if (txids.txids) ids = txids.txids;
    else if (txids.txIds) ids = txids.txIds;
    ...
    ids.forEach(function (txid){
      opts.chain.getTransaction(txid, function (err, resp){
        if (err){
          ...
          response.push({blockId: block_hash, txid: resp.hash, txId: resp.hash});
          if (++count === txs.length){
            callback(null, responseData);
          }
        }
